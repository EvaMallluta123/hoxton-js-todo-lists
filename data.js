const users = [
  ...
  window.users = users
  window.todos = todos
  //This code does not work, waiting to understand this today,I`m sorry for pushing a useless code
 
  
  for (let user of users){
    alert(JSON.stringify(`This is  ${user.name} with the Id  ${user.id},and lives in ${user.address.city}`))
  }
 
// Number.prompt("Select one ID: ") 
// for (let todo of todos){
// if(todo.userid==user.id){ 
//   alert(JSON.stringify(`${user.username} ,${todos.title} `))
// }
// }

let userid= number.prompt("Select one ID: " ) 

for (let todo of todos){
    if(userid==todo.userId)
  alert(`${user.username}, ${todo.title}`)
  }
